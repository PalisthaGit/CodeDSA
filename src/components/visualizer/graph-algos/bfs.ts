...
function bfs(graphData: GraphData, options?: GraphAlgorithmOptions): GraphStep[] {
  ...
  steps.push(buildStep(
    ...
    null, visited, queue, previous, [], new Set(), graphData, true,
  ))
  ...
  while (queue.length > 0) {
    ...
    steps.push(buildStep(
      ...
      `Queue: [${queue.join(', ')}]`,
      currentId, visited, queue, previous, [], new Set(), graphData, true,
    ))
    ...
    for (const { to } of adj[currentId]) {
      ...
      steps.push(buildStep(
        ...
        `Queue: [${queue.join(', ')}]`,
        currentId, visited, queue, previous, [], new Set(), graphData,
      ))
    }
  }
