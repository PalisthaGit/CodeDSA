...
function dijkstra(graphData: GraphData, options?: GraphAlgorithmOptions): GraphStep[] {
  ...
  steps.push(buildStep(
    ...
    null, visited, frontier, distances, previous, [], new Set(), graphData, true,
  ))
  ...
  while (frontier.size > 0) {
    ...
    steps.push(buildStep(
      ...
      currentId, visited, frontier, distances, previous, [], new Set(), graphData, true,
    ))
    ...
    if (currentId === endId) {
      ...
      steps.push(buildStep(
        ...
        null, visited, new Set(), distances, previous, path, pathEdgeIds, graphData, true,
      ))
      steps.push(buildStep(
        'complete',
    ...
    for (const { to, weight } of adj[currentId]) {
      ...
      steps.push(buildStep(
        ...
        `Best to ${to}: ${distances[to] === INF ? '∞' : distances[to]} → ${newDist < distances[to] ? newDist : 'no improvement'}`,
        currentId, visited, frontier, distances, previous, [], new Set(), graphData,
      ))
  ...
  steps.push(buildStep(
    ...
    null, visited, new Set(), distances, previous, [], new Set(), graphData, true,
  ))
