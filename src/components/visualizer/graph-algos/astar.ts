...
function astar(graphData: GraphData, options?: GraphAlgorithmOptions): GraphStep[] {
  ...
  steps.push(buildStep(
    ...
    null, closedSet, openSet, gScore, previous, [], new Set(), graphData, true,
  ))

  while (openSet.size > 0) {
    const currentId = [...openSet].reduce((min, id) => fScore[id] < fScore[min] ? id : min)
    ...
    steps.push(buildStep(
      ...
      `g = actual cost from start, h = heuristic to ${endId}, f = g + h`,
      currentId, closedSet, openSet, gScore, previous, [], new Set(), graphData, true,
    ))
    ...
    if (currentId === endId) {
      ...
      steps.push(buildStep(
        ...
        null, closedSet, new Set(), gScore, previous, path, pathEdgeIds, graphData, true,
      ))
      steps.push(buildStep(
        'complete',
    ...
    for (const { to, weight } of adj[currentId]) {
      ...
      steps.push(buildStep(
        ...
        tentativeG < gScore[to] ? `Improvement found — updating ${to}` : `No improvement`,
        currentId, closedSet, openSet, gScore, previous, [], new Set(), graphData,
      ))
  ...
  steps.push(buildStep(
    ...
    null, closedSet, new Set(), gScore, previous, [], new Set(), graphData, true,
  ))
  return steps
}
