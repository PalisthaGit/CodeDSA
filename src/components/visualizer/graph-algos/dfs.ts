...
function dfs(graphData: GraphData, options?: GraphAlgorithmOptions): GraphStep[] {
  ...
  steps.push(buildStep(
    ...
    null, visited, stack, previous, [], new Set(), graphData, true,
  ))
  ...
  while (stack.length > 0) {
    ...
    steps.push(buildStep(
      ...
      `Stack: [${stack.join(', ')}]`,
      currentId, visited, stack, previous, [], new Set(), graphData, true,
    ))
    ...
    if (currentId === endId) {
      ...
      steps.push(buildStep(
        ...
        null, visited, [], previous, path, pathEdgeIds, graphData, true,
      ))
      steps.push(buildStep(
        'complete',
    ...
    for (const { to } of adj[currentId]) {
      ...
      if (!(to in previous)) previous[to] = currentId
      stack.push(to)
      ...
      steps.push(buildStep(
        ...
        `Stack: [${stack.join(', ')}]`,
        currentId, visited, stack, previous, [], new Set(), graphData,
      ))
    }
  }
  ...
  steps.push(buildStep(
    ...
    null, visited, [], previous, [], new Set(), graphData, true,
  ))
